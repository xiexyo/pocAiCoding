...
public class AccountServiceImpl implements AccountService {
    ...
    public OpenAccountResponse openAccount(OpenAccountRequest request) {
        CustomerEntity customer = customerRepository.findById(request.getCustomerId())
                .orElseThrow(() -> new BizException("CUST404", "客户不存在"));

        ProductEntity product = productRepository.findById(request.getProductCode())
                .orElseThrow(() -> new BizException("PROD404", "产品不存在"));
    ...
    public AccountDetailResponse getAccountDetail(String accountNo) {
        AccountEntity account = accountRepository.findById(accountNo)
                .orElseThrow(() -> new BizException("ACCT404", "账户不存在"));

        CustomerEntity customer = customerRepository.findById(account.getCustomerId())
                .orElseThrow(() -> new BizException("CUST404", "账户对应客户不存在"));

        ProductEntity product = productRepository.findById(account.getProductCode())
                .orElseThrow(() -> new BizException("PROD404", "账户对应产品不存在"));
    ...
    private void checkClassOneAccountLimit(String customerId) {
        List<AccountEntity> accountList = accountRepository.findByCustomerId(customerId);
        for (AccountEntity account : accountList) {
            ProductEntity openedProduct = productRepository.findById(account.getProductCode()).orElse(null);
    ...
    private String generateAccountNo() {
        return "62" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))
